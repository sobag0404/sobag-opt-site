...
- [x] Rust orders/briefs production re-cutover: production exact `/api/orders` and `/api/briefs` are on Rust after Redis-backed store parity, Nginx backup `/etc/nginx/sites-available/sobag-opt.pre-rust-orders-briefs-20260616T164606Z`, and live order/brief/admin/account visibility smoke.
