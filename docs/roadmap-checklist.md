...
- [x] Rust SSR route rehearsal guard: `tools/rust-ssr-route-rehearsal.mjs` generates exact future Nginx `location = ...` snippets for catalog/search/product, fragments, and content pages, while rejecting generic `/`, API/admin, cart/account/favorites/custom, auth, orders, briefs, and content-write routes.
...
- [x] Rust account/orders/admin route rehearsal guard: `tools/rust-account-route-rehearsal.mjs` generates exact future Nginx `location = ...` snippets per route group and rejects generic `/api`, wildcard `/api/admin`, and blocked admin catalog/import/media/PIM routes.
- [x] Rust auth/account cutover candidate guard: `/api/auth/me` is treated as one coupled `GET+PUT` route because a `GET`-only Nginx cutover would break profile/cart/favorites/saved-cart writes on the same URL; rehearsal and audit now label the route as `auth-me (GET+PUT)`.
- [x] Rust auth/account cutover smoke: `tools/rust-auth-me-cutover-smoke.mjs` simulates the future `/api/auth/me` route switch on a temporary shared file-store, with login/register/logout kept on Node, `GET+PUT /api/auth/me` routed to Rust, Node fallback reading Rust-written state, and unrelated APIs staying on Node.
- [x] Rust auth/account production cutover: production Nginx now routes exact `GET+PUT /api/auth/me` to Rust, while login/register/logout and other auth session writes remain on Node fallback; rollback backup is on the VPS.
- [x] Rust account state parity smoke: deploy shadow smoke now compares Node `/api/auth/me` and Rust `/rust/auth/me` account-state PUT output for cart, favorites, and saved carts from identical temporary fixtures.
- [x] Rust account state preview: `PUT /rust/auth/me` covers temporary cart, favorites, saved carts, profile, and buyer review writes with Node-compatible sanitizing; public `/api/auth/me` is now switched after cutover gates passed.
...
- [x] Rust order/brief creation parity smoke: `tools/rust-orders-write-smoke.mjs` now compares Node `/api/orders` and `/api/briefs` with Rust `/rust/orders` and `/rust/briefs`, including order profile side effects, before any public write route cutover.
- [x] Rust order/brief cutover smoke: `tools/rust-orders-briefs-cutover-smoke.mjs` simulates exact `/api/orders` and `/api/briefs` route cutover on a temporary shared file-store, verifies Rust writes, Node admin/account fallback visibility, and unrelated Node fallback APIs before any production switch.
- [ ] Rust orders/briefs production re-cutover: production `/api/orders` and `/api/briefs` are currently on Node fallback after the Redis-vs-file-store rollback; Rust Redis-backed store parity and Redis fixture smokes are implemented, but exact Nginx re-cutover waits for green GitHub/VPS gates and live order/brief smokes.
...
- [x] Rust admin users cutover smoke: `tools/rust-admin-users-cutover-smoke.mjs` simulates exact `/api/admin/users` route cutover on a temporary shared file-store, verifies Rust list/detail/invite/role/delete, Node account fallback visibility, access guards, validation guards, admin lock, and unrelated Node fallback APIs.
...
3. [planned] Import/PIM 2.0:
  - [x] Import batches with preview report and rollback.
   ...
   - [ ] Real object storage for product images instead of Git/static previews.
     - [x] First slice: object storage adapter, admin image API, product image metadata shape, and stronger ignored local photo folders. Vercel Blob was an earlier provider experiment and is no longer active.
     ...
     - [ ] Bulk CLI/importer photo migration to Blob/S3 and responsive WebP/AVIF variants for large catalogs.
       ...
       - [x] Object storage env packet validator: `docs/object-storage-env-packet.md` and `tools/object-storage-env-packet-audit.mjs` define an ignored no-secret provider readiness packet for S3-compatible settings before real photo migration.
   ...
   - [ ] Separate normalized database for products, variants, images, tags, and imports.
     ...
     - [x] Public catalog source seam: `server-routes/_lib/catalog-source.js` centralizes public catalog/review loading for `/api/catalog`, `/api/catalog-query`, and `/api/catalog-detail`, so a later PostgreSQL source can replace store/static loading without changing the public route contracts.
     - [x] PostgreSQL row adapter prep: `server-routes/_lib/catalog-db-rows.js` maps future snake_case `public_catalog_*`, variant, and image rows into current public card/detail payloads; `tools/catalog-db-rows-smoke.mjs` guards compact-card and detail hydration shape.
     - [x] PostgreSQL query builder prep: `server-routes/_lib/catalog-db-query.js` builds parameterized future `public_catalog_cards` list/count and `public_catalog_products` detail SQL for compact reads with guarded sort/filter/page-size mapping; `tools/catalog-db-query-smoke.mjs` guards placeholders and bounded page size.
     - [x] PostgreSQL source adapter prep: `server-routes/_lib/catalog-db-source.js` composes the DB query builder and row adapter behind a fake-client smoke, returning current `/api/catalog-query` and `/api/catalog-detail` compatible payloads without connecting to PostgreSQL or changing runtime storage.
     ...
     - [x] PostgreSQL write statement prep: `server-routes/_lib/catalog-db-write.js` builds parameterized upsert-only statements for future product, variant, image, image-variant, taxonomy, product-taxonomy, import-batch, and import-batch-row writes without deleting products or changing current runtime storage.
...
5. [planned] Performance:
   - [ ] Server-side search/pagination for 10k+ products.
     ...
     - [x] Rust `auth/me` cutover candidate method guard: shadow smoke now verifies unsupported `POST`/`DELETE` stay `405` on Node and Rust, and route rehearsal documents that the future exact route switch must cover `GET+PUT` together.
     - [x] Rust `auth/me` cutover smoke: temporary proxy smoke proves the exact future routing shape keeps auth session writes on Node while routing account-state `GET+PUT` through Rust.
     - [x] Rust `auth/me` production route switch: exact `/api/auth/me` is live on Rust after no-write public validation; Node remains fallback for login/register/logout and other non-switched APIs.
     ...
     - [x] Rust admin content temp-store preview: internal `/rust/admin/content` supports admin/content GET and PUT against the Node-compatible content store, rejects buyer access and invalid payloads; public `/api/admin/content` is now switched after cutover gates passed.
     ...
     - [x] Rust review moderation parity guard: `tools/rust-auth-me-shadow-smoke.mjs` now compares Node vs Rust review PATCH approve/delete output and persisted review lists before any admin content route cutover.
    ...
    - [x] Rust order creation parity guard: `tools/rust-orders-write-smoke.mjs` now compares Node vs Rust order create payloads and buyer profile/account side effects from identical temporary fixtures.
    ...
    - [x] Rust brief creation parity guard: `tools/rust-orders-write-smoke.mjs` now compares Node vs Rust custom-print brief creation payloads from identical temporary fixtures.
...
6. [planned] QA/security/ops:
   ...
   - [x] Vercel deploy/runtime retired: VPS deploys every green push; Vercel deploy scripts and config are absent from the active contour.
