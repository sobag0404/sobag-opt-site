...
- [x] Rust auth-read cutover candidate guard: `tools/rust-auth-me-shadow-smoke.mjs` now verifies Node `/api/auth/me` and Rust `/rust/auth/me` both keep unsupported `POST`/`DELETE` at `405` before any exact `GET /api/auth/me` switch.
...
5. [planned] Performance:
   - [ ] Server-side search/pagination for 10k+ products.
     ...
     - [x] Rust `auth/me` cutover candidate method guard: shadow smoke now verifies unsupported `POST`/`DELETE` stay `405` on Node and Rust, protecting the future read-only exact route switch.
