...
- [x] Rust account/orders/admin cutover guard: `docs/rust-account-orders-admin-cutover-runbook.md` and `tools/rust-account-orders-admin-cutover-audit.mjs` define/check preview routes, blocked public write/admin routes, per-route-group gates, and rollback before any auth/order/admin Nginx switch.
- [x] Rust admin users read-only preview: `/rust/admin/users` mirrors Node admin/manager user/customer reads in temp-store shadow smoke; public `/api/admin/users` writes and reads still stay on Node until cutover gates are green.
...
5. [planned] Performance:
   - [ ] Server-side search/pagination for 10k+ products.
     ...
     - [x] Rust auth write temp-store preview: internal `/rust/auth/login`, `/rust/auth/register`, `/rust/auth/logout`, and `PUT /rust/auth/me` cover login by email/phone, registration, profile update, session cookie set/clear, and password-field stripping in the deploy shadow smoke without routing public auth writes away from Node.
