...
- [x] Rust account state preview: `PUT /rust/auth/me` now covers temporary cart, favorites, saved carts, profile, and buyer review writes with Node-compatible sanitizing; public `/api/auth/me` still stays on Node until cutover gates are green.
