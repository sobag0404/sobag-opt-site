...
5. [planned] Performance:
   - [ ] Server-side search/pagination for 10k+ products.
     ...
     - [x] Rust `auth/me` shadow smoke: deploy now runs a temporary file-store comparison between Node `/api/auth/me` and Rust `/rust/auth/me` for anonymous, buyer, manager, content, and admin sessions before accepting the release.
