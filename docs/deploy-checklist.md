...
Vercel deploy/runtime не используется: активных Vercel package scripts, deploy config и gate scripts в текущем контуре быть не должно. Не проверять и не деплоить Vercel без отдельной явной команды пользователя.
