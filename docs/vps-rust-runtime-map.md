...
- Post-cutover pricing/reviews contract lives in `docs/backend-pricing-reviews-contract.md`: public price lists are grouped by price category, promo rows are separate, admin imports accept Excel-compatible CSV/parsed rows by group or SKU, and buyer reviews require a confirmed matching order. Promo order-pricing precedence and backend XLSX styling remain explicit future business decisions.
