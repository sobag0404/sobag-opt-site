...
- Current backend hardening slice: admin user mutations now write safe `user_admin_update` audit records on Node and Rust, and admins can read a sanitized audit summary through `GET /api/admin/users?audit=1`. The summary includes action/type/id/actor/target/status/role fields only and excludes passwords, hashes, cookies, tokens, raw import payloads, and private customer payloads. Node unsafe-route and route-specific write rate limits now use the shared store-backed limiter by default with a documented in-process fallback; Rust auth/review write guards use the same store-key family with the existing in-process fallback. `tools/file-store-backup.mjs` now supports backup/restore `--dry-run` evidence without modifying production data. Local coverage includes `tools/api-security-smoke.mjs`, `npm run backup:store:self-test`, and Rust unit tests for audit summary and rate buckets.
