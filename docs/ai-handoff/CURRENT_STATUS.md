...
- Current P0 UI cache slice 2026-06-24: frontend public catalog cache is split by request shape (`catalog-home-summary`, listing initial/page, detail), old unscoped public cache and old home-summary keys are purged without clearing private cart/favorites/profile/order state, and catalog home uses local product counts only when the product cache is trusted as full. Catalog HTML now points at `20260624-public-cache-v3` app/data/content assets. `npm.cmd run check`, `npm.cmd run ui:smoke`, and `git diff --check` passed locally.
