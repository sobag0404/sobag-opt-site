...
- Rust migration is live in staged mode: `sobag-opt-rust` runs under systemd on VPS at `127.0.0.1:3001`; Nginx routes `/api/catalog-query`, `/api/catalog-detail`, exact `/api/orders`, `/api/briefs`, `/api/admin/orders`, `/api/admin/users`, `/api/admin/content`, exact SSR catalog/search/product routes, fragments, and simple content pages to Rust; Node remains fallback for generic root, cart, account, auth, admin catalog/import/media, and non-switched pages;
...
- Rust admin content production cutover:
  ...
  - Node remains fallback for `/`, `/cart`, account/auth, admin catalog/import/media, and non-switched routes;
