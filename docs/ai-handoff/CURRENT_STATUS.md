...
- Current deploy/cache slice: catalog pages now reference `app.js?v=20260622-catalog-counts`, production smoke asserts `/catalog.html` uses that cache-bust value, and catalog-home summary logic rejects page-sized partial category counts such as stale `48 товаров` when the server reports a larger total.
