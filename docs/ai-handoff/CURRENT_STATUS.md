...
- Rust migration is live in staged mode: `sobag-opt-rust` runs under systemd on VPS at `127.0.0.1:3001`; Nginx routes `/api/catalog-query`, `/api/catalog-detail`, exact `/api/auth/me`, `/api/auth/login`, `/api/auth/register`, `/api/auth/logout`, `/api/orders`, `/api/briefs`, `/api/admin/orders`, `/api/admin/users`, `/api/admin/content`, exact SSR catalog/search/product routes, fragments, and simple content pages to Rust; Node remains fallback for generic root, cart, admin catalog/import/media/PIM, and non-switched pages;
