...
Current focus:
- Current canonical URL cleanup: `server.mjs` and local `tools/static-server.mjs` now redirect `/index.html` to `/` with 301, shared header/home links point to `/`, and `tools/canonical-url-smoke.mjs` covers root canonical, `/index.html` redirect, key pages, and root CSS/JS assets.
