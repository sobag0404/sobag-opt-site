...
- Current UI P0 2026-06-26 (pending commit): catalog first paint no longer depends on empty JS-filled containers. `catalog.html` ships visible skeleton category cards, actual/collection/holiday placeholders, and bounded price-list loading copy; category tiles also expose `.catalog-category-card` for live smoke compatibility. Critical Lucide icons are now local (`components/app-icons.js`) and all public HTML pages use it instead of the external `unpkg.com` script; SW/site-shell cache versions moved to `20260626-local-icons`. Local checks passed: `npm.cmd run check`, `npm.cmd run ui:smoke` 28/28, `npm.cmd run smoke:frontend-speed`, `git diff --check`; visual QA screenshots are under `output/visual-qa/2026-06-26T09-56-18-589Z/` and should not be committed.
- Current P0 cache/SW pass 2026-06-25: service-worker public API handling now serves fresh cached `/api/catalog-query`, `/api/catalog-detail`, and `/api/price-list` responses without a repeat network fetch during the browser-cache TTL, while stale entries still revalidate/fallback and private/auth/admin/order/account routes remain bypassed. `sw.js` and `components/site-shell.js` cache versions were bumped to `20260625-sw-cache-v2`; root HTML pages now reference the new site shell, production smoke asserts the current site-shell cache-bust, and `tools/sw-cache-behavior-smoke.mjs` covers repeat public API/detail/image cache hits plus private no-cache bypass.
- Current RUM/cache warmup pass 2026-06-25: added first-party Core Web Vitals collection (`/api/rum`) with sanitized sampled public-route metrics (LCP, CLS, INP, FCP, TTFB, NAV_LOAD), no credentials, no raw URLs/PII, no-store responses, store-backed rate limiting, and a local RUM smoke. Deploy cache warmup now covers all manifest-listed safe static public pages, uses `GET` for mandatory first-view product images, and runs bounded background catalog product-page/image batches; private/admin/account/cart/order data remains excluded.
- Current live UI verification 2026-06-25: added a public-route Playwright live UI smoke and GitHub Actions workflow (`live-ui-smoke`) for `https://sobag-shop.online`. It checks root shell/catalog navigation, catalog home category counts without stale page-sized `48`, category-to-catalog SPA navigation, product cards/images, cart shell, and service-worker console/runtime errors without using secrets or authenticated routes.
- Current security tail 2026-06-25: GitHub-owned workflow actions are pinned to full commit SHAs (`actions/checkout`, `actions/setup-node`, `actions/setup-python`, `actions/upload-artifact`). Local `npm audit` remains blocked by this shell's registry network path, but a network-enabled VPS audit against the deployed package lock returned 0 total vulnerabilities.
- Current cache warmup pass 2026-06-24: deploy warmup manifest covers public catalog/search/product SSR surfaces, representative category/search APIs, discovered catalog-detail APIs, discovered `/product?baseSku=...` pages, versioned JS/CSS, and product image warmup while preserving private/auth/admin/order no-store probes. The 2026-06-25 update switched image warmup from `HEAD` to bounded `GET` and added a background catalog image batch.
- Current security pass 2026-06-24: added `tools/security-posture-audit.mjs` and `audit:security-posture` to AutoFix. It gates CI/CD secret-leak patterns, explicit workflow permissions, npm lockfile source/integrity hygiene, tracked secret-like files, and redacted web-log attack-class detection. Local npm audit network is unavailable in this shell, so advisory freshness still needs a network-enabled `npm audit`; local lockfile/SBOM hygiene passed.
- Current admin/backoffice hardening slice 2026-06-24: Rust admin import-batch `updateExisting` preview now preserves existing product id, visibility/status, image/gallery metadata, and SKU override price metadata when those fields are omitted from the import row, and still guards variant SKU collisions before apply. Coverage: targeted Rust unit, `tools/rust-admin-import-batches-cutover-smoke.mjs --self-test`, `tools/api-security-smoke.mjs`, `tools/price-groups-smoke.mjs`, `tools/backend-evidence-smoke.mjs --self-test`, `cargo fmt --check`, `cargo check --locked`, `npm.cmd run check`, and `git diff --check`.
- Current backend/cache latency slice 2026-06-24: public `/api/price-list` now returns compact grouped JSON/CSV by default, omitting full SKU lists unless `includeSkus=1` is explicitly requested for diagnostics/admin-style verification. `tools/production-performance-smoke.mjs` now enforces a bounded price-list payload and rejects default `rows[].skus`, keeping the cacheable price preview/download endpoint lighter.
- Current P0 UI cache slice 2026-06-24 after `6a5cab1`: frontend public catalog cache is split by request shape (`catalog-home-summary`, listing initial/page, detail), old unscoped public cache and old home-summary keys are purged without clearing private cart/favorites/profile/order state, and catalog home uses local product counts only when the product cache is trusted as full. Phase 2 keeps validated full home-summary counts visible while background revalidation fails/recovers. Catalog HTML now points at `20260624-cache-revalidate` app and `20260624-public-cache-v3` data/content assets. `npm.cmd run check`, `npm.cmd run ui:smoke`, and `git diff --check` passed locally.
- Current P0 UI regression slice 2026-06-24 after `c57b0e9`: SPA navigation from `Подушки` listing back to header `Каталог` now ensures the catalog-home summary is requested/refreshed instead of reusing page-sized listing counts. UI smoke mocks `48` category facet rows, navigates back to catalog home, verifies no `48 товаров`, and waits for full `517` counts. `npm.cmd run check`, `npm.cmd run ui:smoke`, and `git diff --check` passed.
- Current UI slice 2026-06-24 after `424c1a1`: catalog active filter chips no longer render the prominent top `Снять все фильтры` reset; sidebar filters now include compact `Применить фильтр` / `Сбросить фильтр` actions. Sidebar reset clears filters in-place, writes `view=list`, keeps the listing visible, and avoids document reload. `npm.cmd run check`, `npm.cmd run ui:smoke`, and `git diff --check` passed.
...
- Current backend hardening slice: production smoke now covers canonical `/index.html -> /`, `/api/catalog-query?pageSize=1` with non-zero prices, `/api/price-list?format=json` rows, and anonymous `/api/admin/product-images` denial in addition to health and HTML shell checks.
- Current deploy/cache slice: catalog pages now reference the current app.js cache-bust, production smoke asserts `/catalog.html` uses that cache-bust value, and catalog-home summary logic rejects page-sized partial category counts such as stale `48 товаров` when the server reports a larger total. The smoke failure now includes safe `total/pageSize/maxCategoryCount/categoryCountRows` diagnostics.
- Current P0 performance slice 2026-06-23: Rust `/api/catalog-query` now runs cards/count/facet DB reads concurrently, and production performance smoke enforces `/catalog.html` + catalog summary first-load budgets plus catalog API latency. Local GitHub/live HTTP remains blocked from this shell; deploy/live confirmation must come from Actions/VPS or a network-enabled environment.
- Current backend hardening slice 2026-06-23: admin price import smoke now covers admin-only access, template download, CSV preview/apply history, and duplicate SKU rejection. Production smoke covers both JSON and downloadable CSV price-list responses and fails if catalog-query category counts look page-limited instead of full-catalog totals.
- Current backend hardening slice 2026-06-23: `tools/api-security-smoke.mjs` now explicitly covers `/api/admin/prices` burst write limiting so admin price imports keep validation errors before the limiter and return `429 rate_limited` after abuse.
- Current production-smoke hardening slice 2026-06-23: `tools/production-smoke.mjs` now checks anonymous denial across representative Rust-owned protected routes (`/api/auth/me`, admin catalog/content/import/orders/prices/product-images/users) instead of only admin media.
- Current price-list smoke hardening slice 2026-06-23: `tools/production-smoke.mjs` now rejects any zero/invalid price-list row in JSON or CSV, not just the first row.
- Current VPS release audit slice 2026-06-23: `tools/vps-release-audit.mjs` now requires `tools/backend-evidence-smoke.mjs` plus `backup:evidence` / `backup:evidence:self-test`, keeping no-secret backend evidence checks in the release package.
- Current price import smoke slice 2026-06-23: `tools/price-groups-smoke.mjs` now proves SKU-only override preview targets exactly one SKU and promo imports record promo-change history in addition to the existing template/CSV/apply/duplicate coverage.
- Current CSV/XLSX parser safety slice 2026-06-23: route-level admin price CSV preview now rejects spreadsheet formula-like SKU input, and `docs/pricing-admin-export.md` states backend import/export is CSV/parsed-rows only while XLSX remains client-side SheetJS parsing.
- Current review eligibility slice 2026-06-23: `tools/api-security-smoke.mjs` now also rejects a completed-order buyer trying to review a different product/SKU, keeping eligibility tied to the purchased item.
- Current account/order ownership slice 2026-06-23: API security smoke now explicitly rejects anonymous account/cart writes and anonymous buyer order PATCH before the existing malformed/cross-account/stale-cart checks.
- Current production-smoke cache slice 2026-06-23: `tools/production-smoke.mjs` now rejects aggressively cached HTML and requires public cache-control on public price-list JSON/CSV responses.
- Current backend evidence slice 2026-06-23: `tools/backend-evidence-smoke.mjs` now supports `--packet <no-secret-evidence-packet.json> --strict` for post-Rust PostgreSQL/MinIO evidence summaries with provider class, backup path class, row/object counts, compact audit types, and import statuses only.
- Current backend hardening slice: `docs/rust-account-orders-admin-cutover-runbook.md` and `tools/rust-account-orders-admin-cutover-audit.mjs` now reflect the completed Rust exact-route cutover instead of the obsolete pre-cutover "do not switch yet" map.
- Current backend hardening slice: buyer review eligibility now has a dedicated contract audit (`tools/review-eligibility-contract-audit.mjs`) in AutoFix to keep Node/Rust markers, no-order/duplicate/other-user/pending-order smokes, and review write rate-limit coverage from regressing.
- Current backend hardening slice: audit summary compatibility now handles legacy string actors and structured actor objects and exposes compact entity/result fields. Node/Rust content saves append safe `content_update` audit records. Account/order ownership is now tied to `userEmail` only; customer contact email no longer grants account history, order comment, or review eligibility access. Rust added store-backed write guards for account update, orders, briefs, admin users, and admin content writes. `tools/backend-evidence-smoke.mjs` provides no-secret evidence counts for orders, reviews, price groups, import history, audit logs, and media-image metadata and is included in AutoFix.
- Current backend hardening slice: admin user mutations write safe `user_admin_update` audit records on Node/Rust, admin catalog saves write `catalog_update`, catalog import preview/reject/apply/rollback writes `catalog_import`, and product image upload/delete/mark-unused writes `media_update`. Admins can read a sanitized audit summary through `GET /api/admin/users?audit=1`; audit records include compact actor/action/entity/count/status fields only and exclude passwords, hashes, cookies, tokens, raw import payloads, and private customer payloads. Node unsafe-route and route-specific write rate limits use the shared store-backed limiter by default with a documented in-process fallback; Rust auth/review write guards use the same store-key family with the existing in-process fallback. `tools/file-store-backup.mjs` supports backup/restore `--dry-run` evidence without modifying production data. Local coverage includes `tools/api-security-smoke.mjs`, `npm run backup:store:self-test`, and Rust unit tests for audit/rate buckets.
- Current backend hardening slice: public order creation now uses non-sequential `SO-*` ids and supports scoped idempotency on Node and Rust. `Idempotency-Key`/`X-Idempotency-Key`/`idempotencyKey` retries for the same customer email or phone return the original order with `idempotent: true` instead of creating a duplicate, while cross-customer replay is not accepted; malformed buyer order ids return `400 invalid_order_id`. Account cart writes now merge duplicate SKU quantities, ignore unsafe client cart keys in favor of SKU-safe keys, expose `cartUpdatedAt`, and optionally reject stale `expectedCartUpdatedAt` writes with `409 cart_conflict`. Buyer review creation has a per-buyer write limiter on Node/Rust so review abuse returns `429` without blocking normal account/profile/cart updates. Local coverage includes Rust units for idempotency/malformed ids/cart normalization/review limiter and `tools/api-security-smoke.mjs`.
- Current backend hardening slice: admin price import history/audit is implemented for Node and Rust. `GET /api/admin/prices` returns recent safe history, successful `apply` records `priceImportHistory[]` plus compact `audit[]` summaries, and raw CSV/files/secrets/private payloads are intentionally not stored. This narrows the prior import-history backlog; next hardening remains write conflict handling, broader admin audit coverage, store-backed rate limits, and automated backup evidence.
- Current verified deployment baseline after direct VPS MinIO repair: manual `vps-deploy` run `27816085213` PASS and `production-smoke` run `27816499824` PASS. The repair fixed local MinIO media writes by restoring MinIO data ownership to the MinIO service user, verifying write/stat/delete, updating the app env on the VPS, and restarting Rust without exposing secrets.
- Rust admin media route is now cut over: production exact `/api/admin/product-images` routes to Rust after the live temporary upload/list/delete smoke passed. The deploy workflow now keeps a safe MinIO data-ownership repair guard in `tools/vps-minio-media-policy.sh` so future media deploy gates can recover the same ownership class without printing secrets.
- Post-cutover backend contract is recorded in `docs/backend-pricing-reviews-contract.md`: grouped public price-list export, admin CSV/Excel-compatible price import by group/SKU, promo price rows, and buyer-only review eligibility are server-side contracts. Promo order-pricing precedence and backend XLSX styling remain future business decisions, not implicit defaults.
- Current backend pricing/review hardening: Node/Rust admin price import now accepts spaced Excel-style numeric prices, rejects invalid promo date windows, treats active promo rows as date-windowed, keeps preview non-mutating, and smoke/tests cover transactional rollback on apply failure. Buyer review tests now cover anonymous/no-order/other-user-order/duplicate/pending/canceled rejection and completed-order success. Order pricing still uses the trusted catalog base price until a separate business rule explicitly approves promo precedence for orders.
- Rust admin catalog route is cut over: production exact `/api/admin/catalog` routes to Rust after deploy-gate switching and live anonymous 401 smoke.
- Rust admin import-batches route is cut over: production exact `/api/admin/import-batches` routes to Rust after deploy-gate switching and live anonymous 401 smoke; destructive import write operations still require explicit authenticated dry-run/apply/rollback evidence for future business changes.
...
- Production Rust exact-route ownership now covers auth, orders/briefs, admin orders/users/content/PIM/prices/catalog/import-batches/product-images, catalog/search/product SSR, public content pages, and catalog query/detail APIs. Node remains only as the static/root/cart compatibility fallback and for any explicitly non-switched legacy paths.
- Remaining business decision: promo/order-pricing precedence and XLSX red styling require explicit business rules before changing order totals or adding backend-styled XLSX output beyond the current CSV/Excel-compatible import/export contract.
...
- This Codex thread can currently reach GitHub Actions and the live domain; latest local live checks passed for health, catalog non-zero prices, production smoke, performance/cache smoke, and production storage readiness.
...
- Runtime code baseline is the latest main deployment accepted by manual `vps-deploy` `27816085213`; production-smoke `27816499824` passed after the MinIO repair. Earlier route baselines remain: admin prices `32076fd`, admin PIM `8907ac2`, auth write `743c63e`, deploy-smoke port collision hardening `9932b03`, and Rust Redis store `129740b`.
...
Current focus:
- Rust cutover complete for targeted exact routes. Production Rust ownership covers catalog query/detail, auth, orders/briefs, admin orders/users/content/PIM/prices/catalog/import-batches/product-images, public catalog/search/product SSR, fragments, and public content pages. Node remains only as static/root/cart compatibility fallback and explicit legacy fallback.
- Post-transition security/cache pass is deployed: server-side order repricing, JSON limits, route rate limits, same-origin cookie mutation guard, API security smoke, VPS static cache policy, and live cache/header smokes are green.
- Rust/VPS transition can be handed off with warnings after successful GitHub/VPS gates. External review backlog TASK-001..TASK-015 is post-cutover work unless the user asks to prioritize it.
...
- Rust migration is live: `sobag-opt-rust` runs under systemd on VPS at `127.0.0.1:3001`; Nginx routes `/api/catalog-query`, `/api/catalog-detail`, exact `/api/auth/me`, `/api/auth/login`, `/api/auth/register`, `/api/auth/logout`, `/api/orders`, `/api/briefs`, `/api/admin/orders`, `/api/admin/users`, `/api/admin/content`, `/api/admin/pim`, `/api/admin/prices`, `/api/admin/catalog`, `/api/admin/import-batches`, `/api/admin/product-images`, exact SSR catalog/search/product routes, fragments, and simple content pages to Rust; Node remains fallback for generic root, cart, static shell/assets, and explicit legacy paths;
...
- Rust auth/me production cutover:
  - production Nginx now routes exact `/api/auth/me` to Rust for account-state `GET+PUT`;
  - Node remains fallback for login/register/logout, `/`, `/cart`, admin catalog/import/media, and non-switched routes;
  - rollback backup on the VPS: `/etc/nginx/sites-available/sobag-opt.pre-rust-auth-me-20260612120803`;
  - verified with production smoke, Nginx marker check, and no-write public validation for anonymous GET, guest PUT rejection, and unsupported POST method guard.
- Rust admin content production cutover:
  - production Nginx now routes exact `/api/admin/content` to Rust;
  - Node remains fallback for `/`, `/cart`, login/register/logout, admin catalog/import/media, and non-switched routes;
  - rollback backup on the VPS: `/etc/nginx/sites-available/sobag-opt.pre-rust-admin-content-20260612114617`;
  - verified with production smoke, Nginx marker check, and no-write public validation for unauthenticated admin content GET/PATCH.
- Rust admin content cutover smoke:
  - added `tools/rust-admin-content-cutover-smoke.mjs` and `npm run smoke:rust:admin-content-cutover`;
  - the smoke uses a temporary shared file-store and local proxy to simulate exact public `/api/admin/content` route cutover to Rust;
  - it verifies admin/content reads and PUT writes, Node `/api/content` fallback visibility, review hide/delete moderation, access/validation guards, and unrelated Node fallback APIs;
  - VPS deploy now runs it after the admin users cutover smoke and before accepting the Rust release.
- Rust admin users production cutover:
  - production Nginx now routes exact `/api/admin/users` to Rust;
  - Node remains fallback for `/`, `/cart`, account/auth, admin content/catalog/import/media, content writes, and non-switched routes;
  - rollback backup on the VPS: `/etc/nginx/sites-available/sobag-opt.pre-rust-admin-users-20260612105515`;
  - verified with production smoke, Nginx marker check, and no-write public validation for unauthenticated admin user read/PATCH.
...
- Rust admin orders production cutover:
  ...
  - rollback backup on the VPS: `/etc/nginx/sites-available/sobag-opt.pre-rust-admin-orders-20260612101545`;
  - verified with production smoke, Nginx marker check, and no-write public validation for unauthenticated admin read/PATCH.
- Rust admin orders cutover smoke:
  - added `tools/rust-admin-orders-cutover-smoke.mjs` and `npm run smoke:rust:admin-orders-cutover`;
  - the smoke uses a temporary shared file-store and local proxy to simulate exact public `/api/admin/orders` route cutover to Rust;
  - it verifies admin/manager reads and PATCH updates through Rust, Node account fallback seeing safe buyer-visible order state, access/validation guards, and unrelated Node fallback APIs;
  - VPS deploy now runs it after the orders/briefs cutover smoke and before accepting the Rust release.
...
- Rust orders/briefs cutover smoke:
  ...
  - it verifies order creation, buyer order message PATCH, minimum-total validation, custom-print brief creation, Node admin fallback visibility, Node account fallback order/profile side effects, and unrelated Node fallback APIs;
  - VPS deploy now runs it after `tools/rust-orders-write-smoke.mjs` and before accepting the Rust release.
...
- Rust SSR production content-page cutover:
  ...
  - rollback backup on the VPS: `/etc/nginx/sites-available/sobag-opt.pre-rust-content-ssr-20260612075815`;
  - added `tools/rust-public-route-smoke.mjs` / `npm run smoke:rust:public-routes` to verify public Rust markers, no preview labels/mojibake, and Node fallback routes.
...
- Rust admin content write parity smoke:
  ...
  - public `/api/admin/content` still stays on Node and no production route/data cutover was made.
...
- Rust admin users temp-store write preview:
  ...
  - public `/api/admin/users` remains on Node and no production route/data cutover was made.
...
- Current Rust admin prices production cutover: exact `/api/admin/prices` now proxies to Rust. Rollback backup: `/etc/nginx/sites-available/sobag-opt.pre-rust-admin-prices-20260616T205931Z`. Verified with GitHub `autofix-check` `27647385195`, `rust-check` `27647385766`, `vps-deploy` `27647440957`, `production-smoke` `27647708702`, live price-group list, safe same-price apply, zero-price rejection, non-zero catalog-detail prices, production smoke, performance/cache smoke, and storage readiness. Remaining Rust fallback zone: admin catalog/import/media mutation routes.
- Current Rust auth write production cutover: exact `/api/auth/login`, `/api/auth/register`, and `/api/auth/logout` now proxy to Rust. Rollback backup: `/etc/nginx/sites-available/sobag-opt.pre-rust-auth-write-20260616T174643Z`. Verified with GitHub `autofix-check` `27637574364`, `rust-check` `27637574179`, `vps-deploy` `27637632119`, `production-smoke` `27637747742`, live register/login/logout/session cookie smoke, CSRF-origin rejection, no-order review guard, health/catalog price smoke, order/brief write smoke, production performance/cache smoke, and storage readiness. Remaining Rust fallback zone: admin catalog/import/media mutation routes.
