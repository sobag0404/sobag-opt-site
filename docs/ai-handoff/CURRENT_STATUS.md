...
- Current cache warmup pass 2026-06-24: deploy warmup manifest now covers public catalog/search/product SSR surfaces, representative category/search APIs, discovered catalog-detail APIs, discovered `/product?baseSku=...` pages, versioned JS/CSS, and first-screen/discovered product images via `HEAD` while preserving private/auth/admin/order no-store probes. Local `cache-warmup-smoke --self-test`, cache architecture audit, performance self-test, and diff-check passed; local outbound production fetch remains blocked, so live expanded warmup proof must come from VPS deploy/production smoke.
