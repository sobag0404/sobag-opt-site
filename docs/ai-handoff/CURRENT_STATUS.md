...
- Latest deployed release marker: `20260616T192544Z-8907ac2`.
...
- Production `/api/auth/login`, `/api/auth/register`, `/api/auth/logout`, `/api/auth/me`, `/api/orders`, `/api/briefs`, and `/api/admin/pim` route exact paths to Rust; Node remains fallback for non-switched root/cart and admin catalog/import/media/price mutation routes.
...
- Runtime code is at release `20260616T192544Z-8907ac2`; Rust admin PIM PostgreSQL fallback was finalized in `8907ac2`, auth write parity fix in `743c63e`, deploy-smoke port collision hardening in `9932b03`, and the Rust Redis store code in `129740b`.
...
- Rust migration is live in staged mode: `sobag-opt-rust` runs under systemd on VPS at `127.0.0.1:3001`; Nginx routes `/api/catalog-query`, `/api/catalog-detail`, exact `/api/auth/me`, `/api/auth/login`, `/api/auth/register`, `/api/auth/logout`, `/api/orders`, `/api/briefs`, `/api/admin/orders`, `/api/admin/users`, `/api/admin/content`, `/api/admin/pim`, exact SSR catalog/search/product routes, fragments, and simple content pages to Rust; Node remains fallback for generic root, cart, admin catalog/import/media/price mutation routes, and non-switched pages;
