...
5. Orders and briefs:
   ...
   - Current status: internal `/rust/admin/orders` and `/rust/admin/users` previews return the current admin/manager order-list and user/customer payloads; shadow smoke compares read payloads with Node `/api/admin/orders` and `/api/admin/users`, and Rust temp-store smoke covers admin user invite/role/delete writes. Production writes still stay on Node.
