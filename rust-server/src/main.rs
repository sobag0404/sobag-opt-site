...
pub(crate) fn row_i64(row: &sqlx::postgres::PgRow, column: &str) -> i64 {
...
pub(crate) async fn load_current_user_from_file_store(
    cookie_header: &str,
...
pub(crate) fn can_edit_content(user: &Value) -> bool {
    matches!(
        user.get("role").and_then(Value::as_str),
        Some("admin" | "content")
    )
