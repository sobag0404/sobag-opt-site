...
- Latest order persistence hardening: Node/Rust order creation uses non-sequential `SO-*` ids, accepts scoped idempotency keys for safe retry without duplicate orders, rejects malformed buyer order ids with `invalid_order_id`, and covers the behavior in `tools/api-security-smoke.mjs` plus Rust unit tests.
