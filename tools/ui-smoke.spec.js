...
test("catalog filters, product modal, variants, and cart stay coherent", async ({ page }) => {
  ...
  await expect(page.locator(".product-detail__copy .product-rating-button")).toBeVisible();
  await expect(page.locator(".sku-line .product-rating-button")).toHaveCount(0);
  await expect(page.locator(".product-rating-button")).toHaveAttribute("aria-label", /Рейтинг:/);
  await expect(page.locator(".product-rating-button")).toHaveAttribute("aria-expanded", "false");
  await expect(page.locator("#productReviewsPanel")).toBeHidden();
  await page.locator(".product-rating-button").click();
  await expect(page.locator(".product-rating-button")).toHaveAttribute("aria-expanded", "true");
