...
async function resolveSmokeProductKey(baseUrl, explicitProductKey) {
  ...
  assert(productKey, "media smoke needs a public catalog product key for upload policy compatibility");
  return productKey;
}
...
async function liveSmoke(args) {
  ...
  const productKey = await resolveSmokeProductKey(args.baseUrl, args.productKey);
  let uploaded = null;
  try {
    const upload = await requestJson(args.baseUrl, "/api/admin/product-images", {
      method: "POST",
      cookie: login.cookie,
      body: mediaFixture(productKey),
    });
    ...
    assert(uploaded.storageKey.startsWith(`products/${productKey}/`), "upload storage key should stay under product prefix");

    const list = await requestJson(
      args.baseUrl,
      `/api/admin/product-images?product=${encodeURIComponent(productKey)}`,
      { cookie: login.cookie },
    );
