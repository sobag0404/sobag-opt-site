...
function parseArgs(argv = process.argv.slice(2)) {
  const args = { source: DEFAULT_SOURCE, dest: DEFAULT_DEST, restore: "", target: "", force: false, selfTest: false };
...
async function backupFileStore(sourceDir, destRoot) {
...
async function main() {
  ...
  const result = args.selfTest
    ? await runSelfTest()
    : args.restore
    ? await restoreFileStore(args.restore, args.target || args.source, { force: args.force })
    : await backupFileStore(args.source, args.dest);
  console.log(
    result.mode === "restore"
